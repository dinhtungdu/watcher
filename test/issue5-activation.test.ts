...
function pane(overrides: Partial<AgentPane> = {}): AgentPane {
  return {
    ...
    target: tmuxTarget({ paneId: '%42', sessionName: 'agents', windowIndex: '3', paneIndex: '2', paneCurrentPath: '/repo' }),
    cwd: '/repo',
...
test('activation works for grouped repo and path fallback rows', () => {
  const gitPane = pane({ id: '%10', target: tmuxTarget({ paneId: '%10', sessionName: 'repo', windowIndex: '1', paneIndex: '0' }) });
  const fallbackPane = pane({ id: '%11', target: tmuxTarget({ paneId: '%11', sessionName: 'tmp', windowIndex: '2', paneIndex: '1' }), git: undefined, cwd: '/tmp/spike' });
...
test('selected-pane detail shows the terminal target activation will use', () => {
